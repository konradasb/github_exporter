...
func main() {
	...
	kingpin.Flag("log.level", "Output log serverity").Default(defaultLogLevel).StringVar(&opts.logLevel)
	kingpin.Flag("web.listen.host", "Host on which to expose metrics").Default(defaultWebListenHost).StringVar(&opts.webListenHost)
	kingpin.Flag("web.listen.port", "Port on which to expose metrics").Default(defaultWebListenPort).StringVar(&opts.webListenPort)
	kingpin.Flag("web.metrics.path", "Path to HTTP metrics").Default("/metrics").StringVar(&opts.webMetricsPath)
	kingpin.Flag("web.healthz.path", "Path to HTTP healthz").Default("/healthz").StringVar(&opts.webHealthzPath)
	kingpin.Flag("web.version.path", "Path to HTTP version").Default("/version").StringVar(&opts.webVersionPath)
	kingpin.Flag("collectors", "List of enabled collectors").Default(defaultCollectors).StringVar(&opts.collectors)
	kingpin.Flag("server.read-timeout", "Server read timeout duration").Default("30s").DurationVar(&opts.serverReadTimeout)
	kingpin.Flag("server.write-timeout", "Server write timeout duration").Default("30s").DurationVar(&opts.serverWriteTimeout)
	kingpin.Flag("github.organizations", "Github organizations to scrape").Default("").StringVar(&opts.githubOrganizations)
	kingpin.Flag("github.repositories", "Github repositories to scrape").Default("").StringVar(&opts.githubRepositories)
